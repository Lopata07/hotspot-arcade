...
// Core 3.x LEDC: channels are assigned by the driver and addressed by PIN, so there
// is no ledcSetup()/ledcAttachPin() pair and no channel number to pick any more.
// (This code was written against the 2.x API, which core 3.x dropped outright.)
static void ledInit() {
    ledcAttach(LED_PIN_RED, LED_FREQ_HZ, LED_RES_BITS);
    ledcAttach(LED_PIN_GREEN, LED_FREQ_HZ, LED_RES_BITS);
    ledcAttach(LED_PIN_BLUE, LED_FREQ_HZ, LED_RES_BITS);
}
...
static void ledSet(uint8_t r, uint8_t g, uint8_t b) {
    ledcWrite(LED_PIN_RED, 255 - ((uint32_t)r * LED_MAX_DUTY) / 255);
    ledcWrite(LED_PIN_GREEN, 255 - ((uint32_t)g * LED_MAX_DUTY) / 255);
    ledcWrite(LED_PIN_BLUE, 255 - ((uint32_t)b * LED_MAX_DUTY) / 255);
}
