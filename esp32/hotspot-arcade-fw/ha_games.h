...
public:
    ...
    void contentClear() {
        ...
        _sfPackCount = 0;
        for(int i = 0; i < TRIVIA_MAX_TOPICS; i++) _punchPacks[i] = WordPack{};
        _punchPackCount = 0;
        _packGame = 0;
    }
    ...
    void contentPack(uint8_t game, const char* name) {
        ...
        if(game == HA_GAME_TRIVIA) {
            ...
            if(_punchPackCount < TRIVIA_MAX_TOPICS) {
                _punchPacks[_punchPackCount] = WordPack{};
                _punchPacks[_punchPackCount].name = name;
                _punchPackCount++;
            }
        }
    }
...
private:
    ...
    // ha_game_punchline.h is included here, ahead of the union, because it defines
    // struct PunchState: a union member needs a *complete* type at its declaration
    // point. Its method bodies may reference members declared further down (they get
    // complete-class-context parsing), but the struct itself cannot wait.
